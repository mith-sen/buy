...
st.set_page_config(page_title="Product Scan · BeforeYouBuy", page_icon="📦", layout="wide")
...
st.markdown(
    """
<div class="page-hero">
  <div class="page-eyebrow">📦 Product Scan</div>
  <div class="page-title">Upload & Analyze a Label</div>
  <p class="page-sub">Upload a photo of a product label/ingredients panel. We'll extract the text and generate health, eco impact, and alternatives.</p>
</div>
""",
    unsafe_allow_html=True,
)
...
st.markdown('<p class="step-title">Upload a Product Label Image</p>', unsafe_allow_html=True)

img = st.file_uploader("Label image", type=["png", "jpg", "jpeg", "webp"], label_visibility="collapsed")
...
if img is not None:
    ...
    if st.session_state.get("product_img_hash") != img_hash:
        st.session_state["product_img_hash"] = img_hash
        st.session_state.pop("product_extracted", None)
        st.session_state.pop("product_result", None)
...
if img and analyze:
    ...
    with st.spinner("📦 Extracting text from label..."):
        try:
            ...
            st.session_state["product_extracted"] = extracted
...
if st.session_state.get("product_extracted") and st.session_state.get("product_result"):
    extracted = st.session_state["product_extracted"]
...
with st.expander("📜 View My Scan History"):
    ...
    if history:
        ...
        st.info("No scans yet. Upload a label to get started!")
